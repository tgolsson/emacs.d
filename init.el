...
(defmacro to/easy-hook (hook prog &optional local append)
    `(add-hook ,hook (lambda ()
                         (interactive)
                         ,prog)
         (when (boundp 'local) local)
...
;; General input setqs
;;
(setq-default indent-tabs-mode nil fill-column 80)
...
(measure-time "Starting server:" (when (and (fboundp 'server-running-p)
